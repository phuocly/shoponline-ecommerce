            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-sm-9 col-right">
                        ...
                        <ol id="products-list" class="products-list">
                            <li class="item">
                                <div class="item-inner">
									...
									if (!empty ($_GET["mahangsanpham"]))
									{	
										$sql = "SELECT * FROM sanpham where MaHangSanPham = ".$_GET["mahangsanpham"] ." LIMIT $batdautu, $sosp1trang" ;										
									}
									else
									{
										$sql = "SELECT * FROM sanpham LIMIT $batdautu, $sosp1trang";										
									}
									$query = mysql_query($sql);									
									?>
                                	<?php if(!empty($query)) while ($r = mysql_fetch_assoc($query))
									{ 
                                        ...
                                        <div class="col-sm-4 col-sms-4 col-smb-12">
											...
											?>
                                                <div class="product_icon">                                                   
                                                    ...
                                                    <?php
													$sqlgiagiam ="SELECT GiaGiam FROM chitietkhuyenmai WHERE MaSanPham = ".$r['MaSanPham']; 
													$querygiagiam = mysql_query($sqlgiagiam);													
													while ($rgg = mysql_fetch_assoc($querygiagiam))
													{
													?>
                                                    <div class='new-icon'><span>-<?php echo number_format ($rgg['GiaGiam'],0," ",'.'); ?></span></div>
                                                    <?php
													}
													?>
                                                </div>                                             	
						...
						if (!empty ($_GET["mahangsanpham"]))
						{
							$sqldemsp = mysql_query("SELECT MaSanPham FROM sanpham where MaHangSanPham = ".$_GET["mahangsanpham"]);
							$laymahang = $_GET["mahangsanpham"];
						}
						else
						{
							$sqldemsp = mysql_query("SELECT MaSanPham FROM sanpham");
						}	
						...
						for($t=1; $t<=$sotrang; $t++)
						{
							...
							{ 
							?>
								<a href="product_list.php?mahangsanpham=<?php echo $laymahang ?>&page=<?php echo $t ?>">
							...
							{
						?>
                                <a href="product_list.php?page=<?php echo $t ?>">
                                <button class="btn btn-danger" title="page" type="button"><?php echo $t ?></button>
                                </a>
								<!-- echo "<a href='product_list.php?page=$t'> Trang $t -</a> "; -->       
                        <?php 
							}
						}
						?>
