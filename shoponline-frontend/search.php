            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-sm-9 col-right">
						...
						{
							$search = addslashes($_GET['search']); // Gán hàm addslashes để chống sql injection
							$sql = "SELECT * FROM sanpham WHERE TenSanPham like '%$search%' LIMIT $batdautu, $sosp1trang";
							$query = mysql_query($sql);
							$sqldem = "SELECT MaSanPham FROM sanpham WHERE TenSanPham like '%$search%' ";
							$querydem = mysql_query($sqldem);
							$num = mysql_num_rows($querydem); // Đếm số dòng trả về khi truy vấn
								
							if (empty($search))
							{
								echo "Bạn cần nhập tên sản phẩm cần tìm kiếm";	
							}
							else
							{
								if ($num > 0 && $search != "")
								{
									echo "có $num sản phẩm với từ khóa <b>$search</b>";			
						...
						$sqldemsp = mysql_query("SELECT MaSanPham FROM sanpham WHERE TenSanPham like '%$search%'");	
						...
						{
						?>
                                <a href="search.php?search=<?php echo $search ?>&ok=search&page=<?php echo $t ?>">
